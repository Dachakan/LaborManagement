...
@section('title','入力工事情報確認｜Labor Management')
...
@section('content')
	...
	<form class="form-signin" action="{{ url('newConstruction/complete') }}" method="POST">
		<p>{{ csrf_field() }}</p>
		<h1 class="h3 mb-3 font-weight-normal">入力工事情報確認</h1>
		...
		<h2 class="h5 mb-3 font-weight-normal">入力日時：{{$time}}</h2>
		<button class="btn btn-lg btn-primary" type="submit">確認完了</button>
		<a href="/labor/public/newConstruction/select" class="btn btn-lg btn-dark" role="button">もう一度入力</a>
	</form>
@endsection